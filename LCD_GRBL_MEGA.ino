...
void sendFile(byte fileIndex) {
  ...
  */
  if (!Exit) {
    ...
    // Now it is done.
    lcd.setCursor(16, 0); lcd.print(F("100%"));
...
void resetSDReader() {
  /*
    To make this work You have to change the SD library a bit (just added one line of code)
    Add root.close() to funtion boolean SDClass::begin(boolean SDClass::begin(uint8_t csPin) {} in SD.cpp library
